...
sed -e 's/IPADDRESS/${PRIVATE_IP}/g' -e 's/COMPONENT/${COMPONENT}/g' r53.json > record.json

echo "Creating DNS record for $COMPONENT :"

aws route53 change-resource-record-sets --hosted-zone-id $HOSTEDZONE_ID --change-batch file://sample.json
